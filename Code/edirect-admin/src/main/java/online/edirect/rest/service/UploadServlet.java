...
/**
 * An old-school servlet implementation, just for comparison. Check out
 * {@code UploadController} for a more modern approach.
 * <p>
 *
 * This servlet uses the commons-fileupload's streaming API to read the uploaded
 * file right off of the request. It shows that this is possible in a servlet,
 * even when spring.http.multipart.enabled=true, since, because we're not in a
 * controller, Spring doesn't parse the request itself.
 */
public class UploadServlet extends HttpServlet {
	...
	/**
	 * Overridden with hideous hackery just so we can fetch our
	 * multipartResolver, so we can return info about it.
	 *
	 * @param config
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			// NOSONAR - swallow, this just means we've removed the
			// multipartResolver from our config
		}
	}
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// UploadController.getViaCommonsFileUploadStreamingApi().
			if (!iter.hasNext()) {
